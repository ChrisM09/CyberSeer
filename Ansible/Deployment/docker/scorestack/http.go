...
func request(ctx context.Context, client *http.Client, r Request) (bool, *string, error) {
	...
	fmt.Println("Hello from the new dynamicbeat hopefully.")
	...
	if r.MatchCode && resp.StatusCode != r.Code {
		//return false, nil, fmt.Errorf("Recieved bad status code: %d\n\t%s", resp.StatusCode, string(body))
		return false, nil, fmt.Errorf(string(body))
	}
	...
	return true, &matchStr, fmt.Errorf(string(body))
}
